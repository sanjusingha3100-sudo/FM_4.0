...
import { MapPin, Navigation, RefreshCw, Clock3, Activity, Gauge, AlertTriangle, ShieldCheck, AlertCircle } from 'lucide-react';
 ...
 * Real-time vehicle tracking for supervisors
 */
export function LiveTracking({ onNavigate }) {
  ...
  const counts = useMemo(() => {
    ...
    return { total: vehicles.length, moving, idling, stopped, offline };
  }, [vehicles]);
    ...
    <div className="flex h-screen bg-slate-50">
      <div className="flex-1 relative">
        ...
        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-20">
          <div className="bg-white/95 backdrop-blur-sm px-6 py-4 rounded-xl shadow-lg flex items-center gap-3 border border-slate-100">
            ...
            <button
              ...
              className="ml-2 inline-flex items-center gap-2 text-xs px-3 py-2 rounded-lg bg-blue-50 text-blue-700 border border-blue-100 hover:bg-blue-100 transition"
              disabled={isLoading}
            >
        ...
        <div className="absolute bottom-6 left-6 z-20 bg-white/95 backdrop-blur-sm px-5 py-4 rounded-xl shadow-lg border border-slate-100">
          <h4 className="text-sm text-slate-900 mb-3">Vehicle Status</h4>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-lg font-semibold text-slate-900 flex items-center gap-2">
              <Activity className="h-4 w-4 text-blue-600" /> {counts.total} vehicles
            </p>
          </div>
          <button
            onClick={fetchTelemetry}
          ...
          {filteredVehicles.map((vehicle) => (
            <button
              ...
              className={`w-full text-left p-3 rounded-lg border transition shadow-sm hover:shadow-md ${
                selectedVehicleId === vehicle.id ? 'border-blue-200 bg-blue-50' : 'border-slate-200 bg-white'
              }`}
            >
